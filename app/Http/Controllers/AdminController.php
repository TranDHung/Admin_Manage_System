...
class AdminController extends Controller
{
    ...
    public function resetPassowrdHandler(Request $request) {
        $request->validate([
        ...
        // send email noti changed password 
        $data = array(
            'admin'=>$admin,
        ...
        return redirect()->route('admin.login')->with('success', 'Your pass has been changed. Use your new pass to login into sys');
    }
    ...
    public function changeProfilePicture(Request $request) {
        ...
        $old_pic = $admin->getAttributes()['picture'];
        $file_path = $path.$old_pic;
        $file_name = 'AD_IMG_'.rand(10,10000).$admin->id.time().uniqid().'.jpg';

        $upload = $file->move(public_path($path), $file_name);

        if($upload) {
            if($old_pic != null && File::exists(public_path($path.$old_pic))){
                File::delete(public_path($path.$old_pic));
            }
            $admin->update([
                'picture'=>$file_name
            ]);
    ...
    public function changeLogo(Request $request) {
        ...
        $old_lo = $settings->first()->site_logo;
        $file_path = $path.$old_lo;
        $file_name = 'LOGO_'.uniqid().'.'.$file->getClientOriginalExtension();

        $upload = $file->move(public_path($path), $file_name);

        if($upload) {
            if($old_lo != null && File::exists(public_path($path.$old_lo))){
                File::delete(public_path($path.$old_lo));
            }
            $settings = $settings->first();
            $settings->site_logo = $file_name;
            $update = $settings->save();
            return response()->json(['sts'=>1, 'msg'=>'Logo site has been successfully uploaded!!']);
        }else{
            return response()->json(['sts'=>0, 'msg'=>'Something went wrong!!!!']);
        }
    }
    ...
    public function changeFavicon(Request $request) {
        ...
        $settings = new GeneralSetting();
        $old_lo = $settings->first()->site_favicon;
        $file_path = $path.$old_lo;
        $file_name = 'LOGO_FAVICON'.uniqid().'.'.$file->getClientOriginalExtension();
        ...
        if($upload) {
            ...
            $update = $settings->save();
            return response()->json(['sts'=>1, 'msg'=>'Favicon site has been successfully uploaded!!']);
        }else{
            return response()->json(['sts'=>0, 'msg'=>'Something went wrong!!!!']);
        }
    }
